document.addEventListener('DOMContentLoaded', () => {
    ...
    if(btnAddRequest) {
        btnAddRequest.addEventListener('click', () => {
            ...
            donViData.forEach(d => {
                donViSelect.innerHTML += `<option value="${d.shortName}">${d.fullName}</option>`;
            });

            const nhanSuSelect = document.getElementById('pyc-nhansu');
            nhanSuSelect.innerHTML = '<option value="">-- Chọn Nhân sự --</option>';
            nhanSuData.forEach(n => {
                nhanSuSelect.innerHTML += `<option value="${n.name}">${n.name}</option>`;
            });
            ...
            if (nccSelect) {
                ...
                nccData.forEach(n => {
                    nccSelect.innerHTML += `<option value="${n.name}">${n.name}</option>`;
                });
            }
            ...
            // Initialize Select2 after modal is shown and populated
    ...
    function printRequest(req) {
        ...
        // Chi tiết
        document.getElementById('print-content').innerText = req['Nội dung mua hàng'] || req['Mục đích'] || req['Nội dung yêu cầu'] || '';
    ...
    function renderDonViTable() {
        ...
        donViData.forEach((d, index) => {
            ...
            tr.innerHTML = `
                ...
                <td>${d.mst || ''}</td>
                <td class="text-right">
                    <button class="btn-remove-row edit-donvi" data-index="${index}" style="margin-right: 5px; color: var(--primary-color); border-color: var(--primary-color);">✏️</button>
                    <button class="btn-remove-row del-donvi" data-index="${index}">🗑️</button>
                </td>
            `;
            tbody.appendChild(tr);
        });

        document.querySelectorAll('.del-donvi').forEach(btn => {
            btn.addEventListener('click', (e) => {
        ...
        document.querySelectorAll('.edit-donvi').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const idx = e.currentTarget.getAttribute('data-index');
    ...
    function renderNhanSuTable() {
        ...
        nhanSuData.forEach((ns, index) => {
            ...
            tr.innerHTML = `
                ...
                <td>${ns.email || ''}</td>
                <td class="text-right">
                    <button class="btn-remove-row edit-nhansu" data-index="${index}" style="margin-right: 5px; color: var(--primary-color); border-color: var(--primary-color);">✏️</button>
                    <button class="btn-remove-row del-nhansu" data-index="${index}">🗑️</button>
                </td>
            `;
            tbody.appendChild(tr);
        });

        document.querySelectorAll('.del-nhansu').forEach(btn => {
            btn.addEventListener('click', (e) => {
                if(confirm('Bạn có chắc muốn xoá?')) {
                    const idx = e.currentTarget.getAttribute('data-index');
    ...
    populateNccDatalist();
        ...
        document.querySelectorAll('.edit-nhansu').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const idx = e.currentTarget.getAttribute('data-index');
